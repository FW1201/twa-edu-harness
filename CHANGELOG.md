...
課綱資料擴充至數學，並把抽取設定改為每個領域一份 profile。

### Added

- **`data/curriculum/數.json`** — 數學的核心素養 27 條、學習表現 131 條、
  學習內容 227 條。全庫共 **743 筆指標 / 2 個領域**。
- **`data/curriculum/profiles/*.yml`** — 每個領域一份抽取設定
  （代碼樣式、抽取策略、階段表示法）。
- 建置腳本新增**硬性完整性檢查**：比對 PDF 中出現的代碼與實際抽出的代碼，
  有遺漏就刪除產出並回傳失敗。
- 測試從 84 增至 **99**，涵蓋數學的不同代碼格式與年級式階段。
...
|---|---|---|
| 學習表現 | `5-Ⅳ-2` | `n-Ⅳ-1` |
| 學習內容 | `Ab-Ⅳ-1` | `N-1-1`（用**年級**不是學習階段） |
| 版面 | 兩欄表 | 五欄表，代碼儲存格垂直置中 |

數學若沿用逐行抽取，**40% 的敘述會被截斷成半句**，而且看起來像正常資料。
因此改走表格抽取。

### Fixed

- 參考檔的數學高中素養碼寫成 `數-U-A2`，官方是 **`數S-U-A2`**（含 S）。
  與先前的 `語-`／`國-` 是同一類錯誤：前綴錯了、代碼查不到，反而躲過存在性檢查。
  數學一節已用權威資料重建。

### 英語文已嘗試但未納入

學習表現漏 42/282；學習內容是依學習階段並排的四欄表，多個代碼擠在同一儲存格，
逐行只抓到 28%、表格抽取則把多筆合併成亂碼。完整性檢查直接刪除了產出。

**沒有資料比錯誤資料好**——這是本階段最重要的一條原則。

---

## [4.1.0] — 2026-09-05

課綱 Capability Seam。把 108 課綱從「模型讀的 markdown」變成「模型呼叫的 tool」。

### 🔴 修正前的實測結果

`tw-edu-lesson-plan-108` 產出的教案，抽驗 10 筆課綱對應：
...
其中最嚴重的是科目縮寫寫錯：國語文的代碼前綴是 `國`，技能寫成 `語`
（`語文` 是領域名，代碼用科目名）。正因為前綴錯了、代碼查不到，
這個錯誤反而躲過了所有既有檢查。教師會把這份教案送交課發會。

### Added

- **`data/curriculum/國.json`** — 國語文的核心素養 27 條、學習表現 157 條、
  學習內容 174 條。由 `scripts/build_curriculum_data.py` 從教育部領綱 PDF 抽取，
  **不是人工輸入，也不是模型生成**。來源與授權見 `data/curriculum/SOURCES.md`。
- **`python/twa_curriculum/`** — 查詢層。查不到就回 `None`，絕不猜測。
  羅馬數字寬鬆比對（`5-IV-2` 與 `5-Ⅳ-2` 指向同一筆）。
- **`python/twa_curriculum_mcp/`** — MCP server，三個工具：
  `curriculum_get`（取單一指標）、`curriculum_lookup`（依關鍵詞找代碼）、
  `curriculum_verify`（批次查核，產出前的最後一道關卡）。
- **`shared/curriculum-lookup.md`** — 課綱查核協議，已接入 5 支核心技能。
  含 MCP 不可用時的 fallback：可繼續參考 references，但必須標注未經查核。
- **`scripts/verify_curriculum_codes.py`** — 讓錯誤代碼進不了 repo。
  一上線就抓到 30 處問題。三種失效模式（代碼不存在、敘述被竄改、前綴寫錯）
  都經反向測試確認擋得住。
- **84 個測試**（計畫要求 50+），含 12 個反例——不存在的代碼必須回 `None`。

### Fixed

- `108_core_competencies.md` 與 `108_subject_indicators.md` 的國語文部分
  **大量與領綱不符**（自創序號、把「標音符號與運用」與「識字與寫字」誤併為一類、
  素養碼前綴全錯）。已用權威資料重建，並在檔頭標注其餘領域尚未查核。
- 生成腳本的樣本課綱資料同步為領綱原文。

### 抽取過程處理的三個 PDF 問題

羅馬數字混用（國語文領綱有 37 處用拉丁 `I`/`V` 而非 `Ⅰ`/`Ⅴ`）、
分散對齊在中文字之間插入空白（「從 中 培 養」）、長敘述跨行。

### 已知限制

**目前只有國語文。** 其餘領域的代碼出現在 repo 時，gate 會列為「待查核」但不擋。
先做一個領域把架構跑通、把抽取的邊界情況找出來，再逐一擴充——這是刻意的順序。
